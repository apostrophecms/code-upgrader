...
module.exports = ({ argv }) => {
  ...
  console.log(stripIndent`
    ...
    To review the changes made by the task, type "git status" to see
    the changes to the file layout, and "git diff HEAD" to see the changes
    made to individual files.

    After that you should run "apos-code-upgrader lint" to view additional
    code upgrade concerns requiring manual attention.
  `);
...
function replace({ argv }) {
  const files = [ ...globByExtension('js'), ...globByExtension('html') ];
  const replaces = [
    // Address common aliases that have changed in A3
    [ 'apos.tasks', 'apos.task' ],
    [ 'apos.locks', 'apos.lock' ],
    [ 'apos.versions', 'apos.version' ],
    [ 'apos.images', 'apos.image' ],
    [ 'apos.groups', 'apos.group' ],
    [ 'apos.permissions', 'apos.permission' ],
    [ 'apos.docs', 'apos.doc' ],
    [ 'apos.utils', 'apos.util' ],
    [ 'apos.attachments', 'apos.attachment' ],
    [ 'apos.caches', 'apos.cache' ],
    [ 'apos.files', 'apos.file' ],
    [ 'apos.users', 'apos.user' ],
    [ 'apos.urls', 'apos.url' ],
    [ 'apos.migrations', 'apos.migration' ],
    [ 'apos.templates', 'apos.template' ],
    [ 'apos.videoFields', 'apos.videoField' ],
    [ 'apos.pages', 'apos.page' ],
    // No blanket replace for areas because we don't want to confuse the
    // separate issue of linting the "apos.area" helper by renaming "apos.areas"
    // to that. These are the individual likely offenders
    [ 'apos.areas.isEmpty', 'apos.area.isEmpty' ],
    [ 'apos.areas.getWidgetManager', 'apos.area.getWidgetManager' ],
    [ 'apos.areas.richText', 'apos.area.richText' ],
...
function refactor({ argv }) {
  ...
  if (isSingleSiteProject()) {
    ...
    if (!fs.existsSync('./lib/modules')) {
      // TODO fully parse out the directory setting in index.js, but
    ...
    console.log(stripIndent`
      ...
      manually change "lib/modules" to modules in the index.js file of
      the bundle.
    `);
  } else {
    fail(stripIndent`
  ...
  function processModule(moduleName, moduleFilename, { renameModule }) {
    ...
    if (specialsFound.extend && specialsFound.extend.value === 'apostrophe-pieces') {
    ...
    if (argv['upgrade-required-files']) {
  ...
  function injectHelpersIfNeeded() {
    if (projectHelpersNeeded) {
      if (!fs.existsSync('./lib')) {
        fs.mkdirSync('./lib');
      }
      fs.writeFileSync('./lib/apostrophe-code-migration-helpers.js', fs.readFileSync(path.join(__dirname, '../helpers/index.js'), 'utf8'));
    }
  }
